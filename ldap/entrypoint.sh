...
EOF
    ...
    if [[ -n "$SLAPD_CONFIG_PASSWORD" ]]; then
		echo "Setting slapd config password"
    ...
    if [[ -n "$SLAPD_ADDITIONAL_SCHEMAS" ]]; then
        IFS=, schemas=($SLAPD_ADDITIONAL_SCHEMAS)

        for schema in "${schemas[@]}"; do
            slapadd -n0 -F /etc/ldap/slapd.d -l "/etc/ldap/schema/${schema}.ldif"
        done
    fi
	echo "configure ppolicy"
	
	
cat << EOF > /addppolicy.ldif
...
echo "add ppolicy to conf"
	
#slapadd -n0 -F /etc/ldap/slapd.d -l /addppolicy.ldif

cat << EOF > /confppolicy.ldif
dn: olcOverlay={0}ppolicy,olcDatabase={1}hdb,cn=config
objectClass: olcOverlayConfig
objectClass: olcPPolicyConfig
olcOverlay: ppolicy
olcPPolicyDefault: cn=default,ou=policies
...
#slapadd -n0 -F /etc/ldap/slapd.d -l /confppolicy.ldif

echo "end ppolicy"

    if [[ -n "$SLAPD_ADDITIONAL_MODULES" ]]; then
        IFS=, modules=($SLAPD_ADDITIONAL_MODULES)
		echo "loop on modules"
...
if [[ "$first_run" == "true" ]]; then
	...
	sed -i "/ 66.115.121.1.5 )/a olcAttributeTypes: {9}( 1.2.840.113556.1.4.8 NAME 'userAccountControl' SYNTAX '1.3.6.1.4.1.1466.115.121.1.27' )" /etc/ldap/slapd.d/cn=config/cn=schema/cn={3}inetorgperson.ldif

	sed -i 's/\$ userSMIMECertificate \$/\$ userAccountControl \$ userSMIMECertificate \$/' /etc/ldap/slapd.d/cn=config/cn=schema/cn={3}inetorgperson.ldif

	slapadd -F /etc/ldap/slapd.d -l /add_ou.ldif
